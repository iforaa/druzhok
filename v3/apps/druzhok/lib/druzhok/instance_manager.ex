defmodule Druzhok.InstanceManager do
  ...
  def list do
    ...
    |> Enum.map(fn inst ->
      ...
      %{name: inst.name, model: inst.model, heartbeat_interval: inst.heartbeat_interval, sandbox: inst.sandbox || "local", alive: alive, telegram_token: inst.telegram_token, api_key: inst.api_key, daily_token_limit: inst.daily_token_limit || 0, dream_hour: inst.dream_hour || -1, language: inst.language || "ru"}
    end)
  end
