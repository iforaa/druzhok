defmodule Druzhok.Sandbox.Firecracker do
  ...
  def exec(instance_name, command),
    do: with_client(instance_name, &Druzhok.Sandbox.FirecrackerClient.exec(&1, command))

  @impl true
  def read_file(instance_name, path),
    do: with_client(instance_name, &Druzhok.Sandbox.FirecrackerClient.read_file(&1, path))

  @impl true
  def write_file(instance_name, path, content),
    do: with_client(instance_name, &Druzhok.Sandbox.FirecrackerClient.write_file(&1, path, content))

  @impl true
  def list_dir(instance_name, path),
    do: with_client(instance_name, &Druzhok.Sandbox.FirecrackerClient.list_dir(&1, path))
