  ...
  def sqlite_browser(assigns) do
    ...
    <div class="p-4 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button phx-click="close_db" phx-target={@target} class="text-xs text-gray-400 hover:text-gray-600">Close</button>
      </div>
              ...
              <td :for={{col, val} <- Enum.zip(@db_columns, row)} class="px-3 py-2 font-mono max-w-xs truncate"
                ...
                <%= if @db_editing == {idx, col} do %>
                  <form phx-submit="db_save_cell" phx-target={@target} class="flex gap-1">
                    ...
                    <button type="button" phx-click="db_cancel_edit" phx-target={@target} class="text-gray-400 text-xs">x</button>
                  </form>
                <% else %>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2 items-center text-xs text-gray-400">
          ...
          <button :if={@db_offset > 0} phx-click="db_prev_page" phx-target={@target} class="px-2 py-1 border rounded hover:bg-gray-50">&larr;</button>
          <span>Page <%= div(@db_offset, @db_page_size) + 1 %></span>
          <button :if={@db_offset + @db_page_size < @db_total_rows} phx-click="db_next_page" phx-target={@target} class="px-2 py-1 border rounded hover:bg-gray-50">&rarr;</button>
        </div>
      </div>
    </div>
    """
