defmodule PiCore.Session do
  ...
  defp friendly_error(reason, _instance_name) do
    reason_str = inspect(reason)
    cond do
      String.contains?(reason_str, "timeout") -> "⏱ Request timed out. Try again."
      String.contains?(reason_str, "closed") -> "🔌 Connection lost. Try again."
      String.contains?(reason_str, "refused") -> "🚫 Service unavailable. Try later."
      String.contains?(reason_str, "HTTP error: 429") -> "⏳ Rate limited. Wait a moment."
      String.contains?(reason_str, "HTTP error: 5") -> "💥 Server error. Try later."
      true -> "❌ #{reason_str}"
    end
  end
  ...
  defp response_target(state) do
    if state.caller && state.caller != self(), do: state.caller, else: state.caller
  end
  ...
  defp default_tools do
    [
      ...
      PiCore.Tools.SetReminder.tool(),
      PiCore.Tools.CancelReminder.tool(),
