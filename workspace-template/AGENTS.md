...
- **Отправке файлов** (`send_file`) — отправка файлов пользователю через Telegram (PDF, картинки, документы)
- **Фоновым задачам** (`spawn_worker`) — запуск долгих задач в фоне, пользователь получит результат когда готово

**Важно:** Не отправляй пользователю локальные пути к файлам (типа `/Users/.../file.pdf`) — он не может их открыть. Вместо этого используй `send_file` чтобы отправить файл через Telegram.
